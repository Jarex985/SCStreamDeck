...
{
    private readonly IKeybindingExecutorService _executorService = executorService ?? throw new ArgumentNullException(nameof(executorService));
    private readonly IKeybindingLoaderService _loaderService = loaderService ?? throw new ArgumentNullException(nameof(loaderService));
    ...
    {
        return await _loaderService.LoadKeybindingsAsync(jsonPath, cancellationToken).ConfigureAwait(false);
    }

    public async Task<bool> ExecuteAsync(KeybindingExecutionContext context, CancellationToken cancellationToken = default)
    {
        return await _executorService.ExecuteAsync(context, cancellationToken).ConfigureAwait(false);
    }

    public bool TryGetAction(string actionName, out KeybindingAction? action)
    {
        return _loaderService.TryGetAction(actionName, out action);
    }

    public IReadOnlyList<KeybindingAction> GetAllActions()
    {
        return _loaderService.GetAllActions();
    }

    public IntPtr GetKeyboardLayoutId()
    {
        return _loaderService.GetKeyboardLayoutId();
    }
}
